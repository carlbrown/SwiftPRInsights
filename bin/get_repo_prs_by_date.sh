...
if [ -z "$REPO" -a -z "$START_DATE" ] ; then
  echo "Usage: $0 <org/repo> <start date in '2016-01-01' format>"
fi

REPO_FILE="`echo $REPO | sed -e 's,/,_,g'`"
  
curl -s --head -i -u ${GITHUB_USER}:${GITHUB_API_TOKEN} "https://api.github.com/search/issues?q=NOT+update+NOT+readme+NOT+typo+NOT+xcode+NOT+implement+in:title+type:pr+state:closed+is:merged+merged:>=${START_DATE}+repo:${REPO}&sort=created&order=asc" > $REPO_FILE.headers
...
for ((PAGE=1;PAGE<=$LASTPAGE;PAGE++)); do
  curl -s  -u ${GITHUB_USER}:${GITHUB_API_TOKEN} "https://api.github.com/search/issues?q=NOT+update+NOT+readme+NOT+typo+NOT+xcode+NOT+implement+in:title+type:pr+state:closed+is:merged+merged:>=${START_DATE}+repo:${REPO}&sort=created&order=asc&page=${PAGE}" > ${REPO_FILE}_page_${PAGE}.json 
